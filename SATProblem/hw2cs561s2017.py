...
class DPLL:
    ...
    def unit_symbol_rules(self, clauseSet, unit):
        ...
        :type clauseSet: set
        """
        clauseSet = set([clause for clause in clauseSet if self.not_contained_in_list(clause, unit)])
        clause_return = set()
        for clause in clauseSet:
            literal_list = []
            for literal in clause:
                if literal.complementary_literal() not in unit:
    ...
    def assign_true_value(self, clauseSet, first_literal):
        ...
        return self.unit_symbol_rules(clauseSet, set([first_literal]))
